<x-app-layout>
    ...
    <div class="py-12">
        ...
        <form method="POST" action="{{ route('pembayaran.update',$pembayaran->id_pembayaran) }}" enctype="multipart/form-data">
            @method('PUT') 
            @csrf
            <!--Nisn-->
            <div>
                ...
                <select class="select2bs4" data-placeholder="Pilih Kelas" style="width: 100%;" name="nisn" >
                    @foreach ($siswa as $siswaKelas)
                      <option value="{{$siswaKelas->nisn}}" name="nisn" {{$siswaKelas->nisn == $siswaKelas->nisn ? "selected" : "" }}>{{$siswaKelas->nisn.' | '}}{{ $siswaKelas->nis .' | '}} {{ $siswaKelas->nama }}</option>
                      {{-- <option value="{{$siswaKelas->nisn}}" name="nisn"  {{$siswaKelas->nisn == $siswaKelas->nisn ? "selected" : "" }}>{{$siswa->nisn.' | '}}{{ $siswa->nis .' | '}} {{ $siswa->nama }}</option> --}}
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <x-input id="tgl_bayar" class="block mt-1 w-full" type="date" name="tgl_bayar"  value="{{ $pembayaran->tgl_bayar }}" />
            </div>
            ...
            <div>
                ...
                <x-input id="bulan_bayar" class="block mt-1 w-full" type="month" name="bulan_bayar" value="{{ $pembayaran->bulan_bayar }}" />
            </div>

            <!--Tahun Bayar-->
            <div>
                <x-label for="tahun_bayar" :value="__('Tahun Bayar')" />
                <x-input id="tahun_bayar" class="block mt-1 w-full" type="month" name="tahun_bayar" value="{{ $pembayaran->tahun_bayar }}" />
            </div>
